...
function _loadTree(sourceFile) {
  const source = utils.getIDLFile(sourceFile.path());
  const tree = webidl2.parse(source);
  return {tree, source};
}

// This is the first step in supporting callbacks, enums, and dictionaries.
// InterfaceData is written as though it should be able to handle all.
// In fact, these should all be separate classes.
function _interfaceFactory(sourceFile, options = {}) {
  const contents = _loadTree(sourceFile);
  let interfaces = {};
  for (let t of contents.tree) {
    if (t.type === 'eof') { continue; }
    let className = (t.type.toString().split(' '))[0];
    let interfaceClass = TREE_TYPES[className];
    if (interfaceClass) {
      let interfaceInstance = new interfaceClass(t, options);
      interfaces[className] = interfaceInstance;
    } else {
      const msg = `${t.type},${sourceFile.path()},The type contained in this file is not currently processible.`;
      if (global.__logger) {
...
class IDLData {
  constructor(source, options = {}) {
    ...
    if (source.type) {
      ...
      // This is a hacky way of getting the caller, but 'use strict' doesn't allow other ways.
      const e = new Error();
      const lines = e.stack.split('\n');
      const msg = `Caller is passing a complete source instead of a type branch ${lines[2].trim()}.`;
      if (global.__logger) {
        global.__logger.info(msg);
      }
      console.log(msg);
      this._loadTree(source);
    }
  }
...
module.exports.interfaceFactory = _interfaceFactory;
